...
from .forms import CategoryForm, MovieForm, MovieUpdateForm, CategoryUpdateForm, UserRegisterForm, UserLoginForm
...
def user_register():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(f'Пользователь {new_user.username} успешно зареган')
            return redirect(url_for('user_login'))
        else:
            text_list = []
            for field, errors in form.errors.items():
                text_list.append(f'{field} : {", ".join(errors)}')
            flash(f'При регисрации категории произошла ошибка{". ".join(text_list)}', 'Ошибка!')
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if user and user.check_password(password):
                login_user(user)
                flash(f'Вы Успешно вошли в систему', "Успех")
                return redirect(url_for('index'))
            else:
                flash(f'Неверный Login and пароль', "Ошибка")
    return render_template("accounts/index.html", form=form, title=title)
